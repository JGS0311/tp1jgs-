...
double get_network_traffic()
{
    ...
    if (fgets(buffer, sizeof(buffer), fp) == NULL)
    {                                                              // Cambio: Verificar retorno de fgets
        perror("Error leyendo la primera línea de /proc/net/dev"); // Cambio: Manejo de error
        fclose(fp);                                                // Cambio: Cerrar archivo en caso de error
        return -1.0;
    }
    if (fgets(buffer, sizeof(buffer), fp) == NULL)
    {                                                              // Cambio: Verificar retorno de fgets
        perror("Error leyendo la segunda línea de /proc/net/dev"); // Cambio: Manejo de error
        fclose(fp);                                                // Cambio: Cerrar archivo en caso de error
        return -1.0;
    }
